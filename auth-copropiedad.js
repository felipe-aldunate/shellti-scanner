...
async function createRequest({ name, organization, cargo, email, context }) {
  ...
  const rows = await sb('POST', 'copropiedad_requests', {}, {
...
async function getRequests() {
  const rows = await sb('GET', 'copropiedad_requests', { select: '*', order: 'created_at.desc' });
  return rows.map(toRequest);
}

async function getRequest(id) {
  const rows = await sb('GET', 'copropiedad_requests', { select: '*', filter: `id=eq.${id}` });
...
async function approveRequest(id, durationMs, durationLabel) {
  ...
  await sb('PATCH', 'copropiedad_requests', { filter: `id=eq.${id}` }, {
    status: 'approved',
  ...
  const existing = await sb('GET', 'copropiedad_users', {
  ...
  if (existing.length) {
    await sb('PATCH', 'copropiedad_users', { filter: `email=eq.${req.email}` }, userData);
  } else {
    await sb('POST', 'copropiedad_users', {}, userData);
  }
...
async function rejectRequest(id) {
  const rows = await sb('PATCH', 'copropiedad_requests', { filter: `id=eq.${id}` }, { status: 'rejected' });
  return rows[0] ? toRequest(rows[0]) : null;
}

// ── USERS ─────────────────────────────────────────────────────────────────────
async function getUsers() {
  const rows = await sb('GET', 'copropiedad_users', { select: '*', order: 'created_at.desc' });
...
async function revokeUser(email) {
  const rows = await sb('PATCH', 'copropiedad_users', { filter: `email=eq.${email}` }, {
...
async function validateToken(token) {
  ...
  const rows = await sb('GET', 'copropiedad_users', {
  ...
  if (expired) {
    await sb('PATCH', 'copropiedad_users', { filter: `token=eq.${token}` }, { active: false });
    return null;
  }
...
async function incrementSession(token) {
  ...
  await sb('PATCH', 'copropiedad_users', { filter: `token=eq.${token}` }, { sessions_used: newCount });
  return toUser({ ...u, sessions_used: newCount });
}


async function saveSessions(token, sessions) {
  const rows = await sb('PATCH', 'copropiedad_users', { filter: `token=eq.${token}` }, {
